...
def test_configure_sweeps_aliases():
    ...
    for alias in ['local', 'dmrg', 'dmrg-like']:
        ...
        assert isinstance(model.strategy, Sweeps), f"alias '{alias}' should produce Sweeps"
...
def test_predict_input_too_short():
    key = jax.random.PRNGKey(42)
    model = MPS_initialize(L=10, initializer=randn(1e-2), key=key,
                           shape_method='even', bond_dim=3, phys_dim=2, cyclic=False)
    sample = np.random.rand(5)
...
def test_train_unsupervised_global():
    key = jax.random.PRNGKey(42)
    model = MPS_initialize(L=4, initializer=randn(1e-1), key=key,
                           shape_method='even', bond_dim=2, phys_dim=2, cyclic=False)
    model.configure(
        strategy='global',
        optimizer=optax.adam,
        learning_rate=1e-2,
...
def test_evaluate_unsupervised():
    ...
    model.batch_size = 4
    loss_val = model.evaluate(inputs=data, batch_size=4, embedding=TrigonometricEmbedding(),
                              evaluate_type=TrainingType.UNSUPERVISED, metric=metrics.NegLogLikelihood)
