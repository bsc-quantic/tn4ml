...
def test_SMPO_spacing():
    key = jax.random.PRNGKey(42)
    smpo = SMPO_initialize(L=10, initializer=gramschmidt('normal', 1e-3), key=key,
                           shape_method='even', spacing=5, bond_dim=5, phys_dim=(2, 2), cyclic=False)
...
def test_SMPO_normalize():
    key = jax.random.PRNGKey(42)
    smpo = SMPO_initialize(L=10, initializer=randn(1.0), key=key,
                           shape_method='even', spacing=5, bond_dim=5, phys_dim=(2, 2), cyclic=False)
    # Scale up tensors
...
def test_SMPO_apply():
    ...
    key = jax.random.PRNGKey(42)
    smpo = SMPO_initialize(L=10, initializer=orthogonal(), key=key,
                           shape_method='even', spacing=2, bond_dim=4, phys_dim=(2, 2), cyclic=False)
