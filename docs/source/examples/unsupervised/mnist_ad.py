...
warnings.filterwarnings(
    "ignore", message="OMP: Info #276: omp_set_nested routine deprecated"
)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Read arguments for training TN for anomaly detection"
    )
    ...
    parser.add_argument(
        "-embedding", dest="embedding", type=str, default="trigonometric"
    )
    ...
    parser.add_argument(
        "-metric_loss", dest="metric_loss", type=str, default="LogQuadNorm"
    )
    parser.add_argument(
        "-test_batch_size", dest="test_batch_size", type=int, default=64
    )
    ...
    for bond_dim in list(args.bond_dims):
        for spacing in list(args.spacings):
            for init_string in list(args.initializers):
                ...
                # initialize model
                print(
                    "Initializing model with bond dimension: ",
                    bond_dim,
                    " and spacing: ",
                    spacing,
                )
                ...
                sns.histplot(
                    anomaly_score, bins=100, kde=True, color="skyblue", label="anomaly"
                )
                sns.histplot(
                    normal_score, bins=100, kde=True, color="red", label="normal"
                )
