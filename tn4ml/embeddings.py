...
class QuantumBasisEmbedding(Embedding):
    ...
    def __call__(self, x: Any) -> jnp.ndarray:
        ...
        def true_fun(_):
            return jnp.array(self.basis[0])

        def false_fun(_):
            return jnp.array(self.basis[1])
...
class TrigonometricEmbeddingAvg(ComplexEmbedding):
    ...
    def __call__(self, features: Collection) -> jnp.ndarray:
        ...
        """
        return self.embeddings(jnp.mean(jnp.array(features)))
...
class BasePatchEmbedding(StateVectorToMPSEmbedding):
    ...
    def pad_or_truncate_statevector(
        self, statevector: jnp.ndarray, target_size: int
    ) -> jnp.ndarray:
    ...
    def combine_mps_patches(self, mps_patches: Any, n_qubits: int) -> list:
        ...
        for patch in mps_patches:
            for i, arr in enumerate(patch):
                if i == number_interval * n_qubits and len(arr.shape) == 2:
                    ...
                    i == ((number_interval + 1) * n_qubits - 1) and len(arr.shape) == 2
                ):
    ...
    def __call__(self, x: jnp.ndarray) -> qtn.MatrixProductState:
        ...
        if self.k > H:
            raise ValueError(
                f"Patch dimension k = {self.k} is too large for {H}x{W} images."
            )
        ...
        for patch in patches:
            patch_data = (
                patch.ravel()
                if not hasattr(self, "flatten_snake")
                else self.flatten_snake(patch)
            )
...
class PatchEmbedding(BasePatchEmbedding):
    ...
    def flatten_snake(self, image: jnp.ndarray) -> jnp.ndarray:
        ...
        """
        image = jnp.where(
            jnp.arange(image.shape[0])[:, None] % 2 == 1, jnp.flip(image, axis=1), image
        )
    ...
    def create_statevector(self, x: jnp.ndarray) -> Any:
        ...
        statevector = jnp.where(
            color_bits == 0, cos_values[address_indices], sin_values[address_indices]
        )
        ...
        padded_statevector = self.pad_or_truncate_statevector(
            statevector.flatten(), fixed_size
        )
...
def embed(
    ...
    **mps_opts : Any
    ...
    """
    if not issubclass(
        type(phi), (ComplexEmbedding, Embedding, StateVectorToMPSEmbedding)
    ):
        raise TypeError("Invalid embedding type")

    if isinstance(phi, Embedding):
    ...
    elif isinstance(phi, ComplexEmbedding) and x.ndim == 2:
        if isinstance(phi.dims, int):
        ...
        mps = phi(x)  # type: ignore[arg-type]
