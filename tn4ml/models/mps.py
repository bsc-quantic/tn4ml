...
def generate_shape(
    ...
    if method == "even":
        shape = (
            ...
            if class_index is not None and position == class_index
            else (bond_dim, bond_dim, phys_dim)
        )
        if position == 1:
            shape = (
                (1, bond_dim, phys_dim, class_dim)
                if class_index is not None and position == class_index
                else (1, bond_dim, phys_dim)
            )
        if position == L:
            shape = (
                (bond_dim, 1, phys_dim, class_dim)
                if class_index is not None and position == class_index
                else (bond_dim, 1, phys_dim)
            )
    else:
        assert not cyclic
        ...
        if position == 1:
            shape = (
                (1, chir, phys_dim, class_dim)
                if class_index is not None and position == class_index
                else (1, chir, phys_dim)
            )
        elif position == L:
            shape = (
                (chil, 1, phys_dim, class_dim)
                if class_index is not None and position == class_index
                else (chil, 1, phys_dim)
            )
        else:
            shape = (
                (chil, chir, phys_dim, class_dim)
                if class_index is not None and position == class_index
                else (chil, chir, phys_dim)
            )
...
def generate_ind(
    ...
    if len(shape) == 3:
        if position == 1:
            ...
            ind = (
                (f"bond_{position - 2}", f"k{position - 1}", f"b{position - 1}")
                if class_index == position
                else (
                    f"bond_{position - 2}",
                    f"bond_{position - 1}",
...
def MPS_initialize(
    ...
    **kwargs,
    ...
    if arrays is not None:
        ...
        )  # class_index is required when arrays or shapes are provided
    ...
    if class_index is not None:
        ...
        if arrays is not None:
            for i, array in enumerate(arrays):
                ...
                tensors.append(qtn.Tensor(array, inds=ind, tags=tags_id.format(i)))
        else:
                ...
                if callable(initializer) and "rand_unitary" in getattr(
                    ...
                    if add_identity:
                        if len(array.shape) == 3:
                            ...
                            copy_array = copy_array.at[:, :, 0].add(
                                jnp.eye(array.shape[0], array.shape[1], dtype=dtype)
                            )
                            ...
                            if add_to_output:
                                copy_array = jnp.copy(array)
                                identity = jnp.eye(
                                    array.shape[0], array.shape[1], dtype=dtype
                                )
                            ...
                            raise ValueError(
                                "Tensors need to always be 3D or 4D in MPS for classification."
                            )
                    ...
                    if boundary == "obc":
                        ...
                        if i == 1:
                            ...
                            aux_array = aux_array.at[0, :, :].set(array[0, :, :])
                            array = aux_array
                tensors.append(qtn.Tensor(array, inds=ind, tags=tags_id.format(i - 1)))
                ...
                if callable(initializer) and "rand_unitary" not in getattr(
                    ...
                    if add_identity:
                        if len(tensor.shape) == 3:
                            ...
                            copy_tensor.at[:, :, 0].add(
                                jnp.eye(tensor.shape[0], tensor.shape[1], dtype=dtype)
                            )
                            tensor = copy_tensor
                        else:
                            raise ValueError(
                                "There was an error in generating shape. They should be 3D"
                            )
