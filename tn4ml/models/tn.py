...
class TensorNetwork(Model, qtn.tensor_1d.TensorNetwork1DFlat):
    ...
    def canonize(self, where, cur_orthog="calc", info=None, bra=None, inplace=False):
        ...
        self.canonicalize(
            where, cur_orthog=cur_orthog, info=info, bra=bra, inplace=inplace
        )
        ...
        if self.L > 200:  # for large systems
            ...
            if insert is None:
                ...
                if not (0 <= insert < len(self.tensors)):
                    raise IndexError(
                        f"Insert index {insert} is out of bounds for the tensor list."
                    )
...
def TN_initialize(
    ...
    **kwargs,
    ...
    if inds is None:
        raise ValueError(
            "Provide indices for tensors - connectivity map between tensors."
        )
